...
public class GameManager_C11041 : MonoBehaviour, IOAKSGame
{
    ...
    [HideInInspector]
    public List<int> QuestionOrderListtemp1;
    [HideInInspector]
    public List<int> QuestionOrderListtemp2;
    ...
    public void SetTutorial(GameInputData gameInputData)
    {
        ...
        {
            iTween.ScaleFrom(Tut_Items.transform.GetChild(i).gameObject, iTween.Hash("Scale", Vector3.zero, "time", 1f, "delay", _delay+1f, "easetype", iTween.EaseType.easeOutElastic));
            StartCoroutine(PlayAudioAtOneShot(Sound_Ting, _delay+1f));
            _delay += 0.1f;
        }
        PlayAudio(Sound_Intro1, 1.5f);
        Invoke("Tut_ItemsCall",Sound_Intro1[VOLanguage].clip.length);
        Invoke("Tut_ItemsCall1",Sound_Intro1[VOLanguage].clip.length+1f);        
        PlayAudio(Sound_Intro2, Sound_Intro1[VOLanguage].clip.length + 1.5f);
        PlayAudio(Sound_Intro3, Sound_Intro2[VOLanguage].clip.length + 10f);
        Invoke("TutorialObj12", Sound_Intro3[VOLanguage].clip.length + 12f);
        Invoke("CallIntro4", Sound_Intro3[VOLanguage].clip.length + 18f);
        Invoke("EnableAnimator", 3f);
        Invoke("EnableTutNoTRaycatTarget", 22.5f);
    }
    ...
    {
        TutorialObj2.transform.GetChild(2).GetComponent<Image>().raycastTarget = true;
    }
    public void Tut_ItemsCall()
    {
        Tut_Items.gameObject.SetActive(false);
    }
    public void Tut_ItemsCall1()
    {
        Tut_Items1.gameObject.SetActive(true);
        float _delay = 0;
        for (int i = 0; i < Tut_Items1.transform.childCount; i++)
        {
            iTween.ScaleFrom(Tut_Items1.transform.GetChild(i).gameObject, iTween.Hash("Scale", Vector3.zero, "time", 1f, "delay", _delay, "easetype", iTween.EaseType.easeOutElastic));
            StartCoroutine(PlayAudioAtOneShot(Sound_Ting, _delay));
            _delay += 0.1f;
        }
    }
    public void TutorialObj12()
    {
    ...
    public void Selected_TutAnswer()
    {
        ...
        TutorialObj2.transform.GetChild(2).GetComponent<Image>().raycastTarget = false;
        TutorialObj2.transform.GetChild(2).GetComponent<PopTweenCustom>().StartAnim();
        StopRepetedAudio();
    ...
    public void BtnAct_OkTut()
    {
        ...
        TutorialObj2.transform.GetChild(2).GetComponent<PopTweenCustom>().Invoke("StopAnim", LengthDelay + 1f);
        PlayAudio(Sound_Intro7, LengthDelay + Sound_Intro6[VOLanguage].clip.length + 2f);
        Invoke("SetGamePlay", LengthDelay + Sound_Intro6[VOLanguage].clip.length + Sound_Intro4[VOLanguage].clip.length + 4f);
    }
    #endregion
    ...
    public void GenerateLevel()
    {
        ...
        QuestionOrderListtemp1 = new List<int>();
        QuestionOrderListtemp2 = new List<int>();
        for (int i = 0; i < QuestionsOrder1.Length; i++)
        {
            QuestionOrderListtemp1.Add(QuestionsOrder1[i]);
        }
        for (int i = 0; i < QuestionsOrder2.Length; i++)
        {
            QuestionOrderListtemp2.Add(QuestionsOrder2[i]);
        }
        ...
        {            
            ...
            QuestionOrderListtemp1.Remove(QuestionsOrder1[tempq]);
        ...
        if (QuestionOrder2 < (QuestionOrderList1.Count))
        {
            ...
            QuestionOrderListtemp2.Remove(QuestionsOrder2[tempq]);
        ...
        if (WrongAnsweredQuestionOrder1 < (WrongAnsweredQuestions1.Count))
        {
            ...
            QuestionOrderListtemp1.Remove(WrongAnsweredQuestions1[tempq]);
        ...
        if (WrongAnsweredQuestionOrder2 < (WrongAnsweredQuestions2.Count))
        {
            ...
            QuestionOrderListtemp2.Remove(WrongAnsweredQuestions2[tempq]);
    ...
    public void BtnAct_Ok()
    {
        ...
        if (UserAnsr.Contains("Answer"))
        {
           ...
           // MultiLevelManager.instance.UpdateProgress(1, 1);
            ...
            else
            {
                ...
                //Invoke("ShowLC", LengthDelay + LengthDelay2 +3.75f);
                SendResultFinal();
            }
            CancelInvoke("RepeatQVOAftertChoosingOption");
        }
        else
        {
            ...
            {
                float LengthDelay = PlayAnswerVoiceOver(CurrentQuestion-1, 0.25f);
                ...
                {
                    Debug.Log("Questions Finished");
                    //Invoke("ShowLC", Sound_IncorrectAnswer.clip.length + LengthDelay + 5f);
                    SendResultFinal();
                }
                CancelInvoke("RepeatQVOAftertChoosingOption");
                WrongAnsrsCount = 0;
