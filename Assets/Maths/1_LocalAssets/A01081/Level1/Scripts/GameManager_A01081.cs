...
public class GameManager_A01081 : MonoBehaviour, IOAKSGame
{
    ...
    public GameObject[] Tut_AntItems;
    ...
    public void SetTutorial(GameInputData gameInputData)
    {
        ...
        float _delay = 0;
        for (int i = 0; i < Tut_AntItems.Length; i++)
        {
            StartCoroutine(PlayAudioAtOneShot(Sound_Ting, _delay+0.5f));
            iTween.MoveFrom(Tut_AntItems[i].gameObject, iTween.Hash("y", 1500, "time", 0.5f, "delay", _delay, "easetype", iTween.EaseType.linear));
            _delay += 0.2f;           
        }

        PlayAudio(Sound_Intro1, 2f);
        Invoke("EnableAnimator", 7f);
        Invoke("CallIntro2", Sound_Intro1[VOLanguage].clip.length + 2.2f);
    }
    ...
    public void BtnAct_OkTut()
    {
        ...
        Tut_Items.transform.GetChild(0).GetComponent<PopTweenCustom>().Invoke("StopAnim", LengthDelay + 1f);

        PlayAudio(Sound_Intro5, LengthDelay + Sound_Intro4[VOLanguage].clip.length + 2f);

        Invoke("SetGamePlay", LengthDelay + Sound_Intro4[VOLanguage].clip.length + Sound_Intro5[VOLanguage].clip.length + 3f);
    }
    ...
    public void GenerateLevel()
    {
        ...
        LevelHolder[1].transform.parent.gameObject.SetActive(false);
        ...
        for (int i = 0; i < Items1.Length; i++)
        {
            ...
            {               
                ...
                if (CurrentItem == 0)
                {
                    ...
                    CurrentItems[i].GetComponent<Animator>().enabled = true;
                }
                else
                {
        ...
        if (CurrentItem == 0)
        {
            ...
            {
                StartCoroutine(PlayAudioAtOneShot(Sound_Ting, 3f+_delay));
                _delay += 0.1f;
            }
            Invoke("PlayQuestionVoiceOverWithDelay", 4.5f);
        }
        else
        {
            float _delay = 0;
            for (int i = 0; i < CurrentItems.Length; i++)
            {
                //iTween.ScaleFrom(Items1[i].gameObject, iTween.Hash("Scale", Vector3.zero, "time", 1f, "delay", _delay, "easetype", iTween.EaseType.easeOutElastic));
                if (CurrentItem == 1)
                {
    ...
    public void PlayQuestionVoiceOverWithDelay()
    {
        ...
        for (int i = 0; i < CurrentItems.Length; i++)
        {
            CurrentItems[i].GetComponent<Animator>().enabled = false;
            CurrentItems[i].GetComponent<Image>().raycastTarget = true;
        }
    }
    ...
    public void BtnAct_Ok()
    {
        ...
        if (CorrectAnsrIndex == UserAnsr)
        {
            ...
            Invoke("HighlightOptions", LengthDelay);            
        ...
        else
        {
            ...
            if (WrongAnsrsCount >= 2)
            {
                ...
                if (!WrongAnsweredQuestions1.Contains(CurrentQuestion) && QuestionOrder1 <= (QuestionOrderList1.Count) && CurrentItem == 1)
                {
                    WrongAnsweredQuestions1.Add(CurrentQuestion);
                }
                else
                 if (!WrongAnsweredQuestions2.Contains(CurrentQuestion) && QuestionOrder2 <= (QuestionOrderList2.Count) && CurrentItem==0)
                {
                ...
                else
                {
                    ...
                    StartCoroutine(SetActiveWithDelayCall(LevelsHolder.gameObject, false, LengthDelay + 2f));
                    //Invoke("ShowLC", Sound_IncorrectAnswer.clip.length + LengthDelay + 2.5f);
                    SendResultFinal();
                }
                CancelInvoke("RepeatQVOAftertChoosingOption");
                WrongAnsrsCount = 0;
    ...
    void HighlightOptions()
    {
        if (CurrentItem != 0)
        {
            ...
            for (int i = 0; i < CurrentItems.Length; i++)
            {
                ...
                {
                    CurrentItems[i].GetComponent<PopTweenCustom>().Invoke("StartAnim", _delay);
                    _delay += 1;
                    if (i != CorrectAnsrIndex)
                        CurrentItems[i].GetComponent<PopTweenCustom>().Invoke("StopAnim", _delay);
                }
            }
        }
        else
    ...
    void PlayQuestionVoiceOver(int _Qi)
    {
        if (VOLanguage == 0)
        {
            QVOLength = Sound_QVO[CurrentItem].EN_Sound_QVO[CurrentQuestion].clip.length;
            PlayAudioRepeated(Sound_QVO[CurrentItem].EN_Sound_QVO[CurrentQuestion]);
            Debug.Log("Sound : " + CurrentItem);
        }
        if (VOLanguage == 1)
        {
            QVOLength = Sound_QVO[CurrentItem].HI_Sound_QVO[CurrentQuestion].clip.length;
            PlayAudioRepeated(Sound_QVO[CurrentItem].HI_Sound_QVO[CurrentQuestion]);
        }
        if (VOLanguage == 2)
        {
            QVOLength = Sound_QVO[CurrentItem].TL_Sound_QVO[CurrentQuestion].clip.length;
            PlayAudioRepeated(Sound_QVO[CurrentItem].TL_Sound_QVO[CurrentQuestion]);
        }
    ...
    public float PlayAnswerVoiceOver(int _Ai, float _delay)
    {
        ...
        if (VOLanguage == 0)
        {
            ClipLength = Sound_AVO[CurrentItem].EN_Sound_AVO[CurrentQuestion].clip.length;
            PlayAudio(Sound_AVO[CurrentItem].EN_Sound_AVO[CurrentQuestion], _delay);
            Debug.Log("Sound : " + CurrentItem);
        }
        if (VOLanguage == 1)
        {
            ClipLength = Sound_AVO[CurrentItem].HI_Sound_AVO[CurrentQuestion].clip.length;
            PlayAudio(Sound_AVO[CurrentItem].HI_Sound_AVO[CurrentQuestion], _delay);
        }
        if (VOLanguage == 2)
        {
            ClipLength = Sound_AVO[CurrentItem].TL_Sound_AVO[CurrentQuestion].clip.length;
            PlayAudio(Sound_AVO[CurrentItem].TL_Sound_AVO[CurrentQuestion], _delay);
        }
        return ClipLength;
    }
    ...
    public QO_AudioSource_A01081[] Sound_QVO;
    public AO_AudioSource_A01081[] Sound_AVO;
...
[System.Serializable]
public class AO_AudioSource_A01081
{
...
[System.Serializable]
public class QO_AudioSource_A01081
{
