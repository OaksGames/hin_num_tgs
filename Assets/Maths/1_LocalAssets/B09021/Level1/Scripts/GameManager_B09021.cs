...
public class GameManager_B09021 : MonoBehaviour, IOAKSGame
{
    ...
    public void SetTutorial(GameInputData gameInputData)
    {
        ...
        {
            iTween.ScaleFrom(Tut_Items.transform.GetChild(i).gameObject, iTween.Hash("Scale", Vector3.zero, "time", 0.5f, "delay", Sound_Intro1[VOLanguage].clip.length + 2.25f + _delay,"easetype", iTween.EaseType.easeOutElastic));
            StartCoroutine(PlayAudioAtOneShot(Sound_Ting, Sound_Intro1[VOLanguage].clip.length + 2.25f + _delay));
            _delay += 0.1f;
        }

        iTween.MoveTo(Tut_Sidepanel.gameObject, iTween.Hash("x", 512,"islocal",true, "time", 1f, "delay", Sound_Intro1[VOLanguage].clip.length + 2, "easetype", iTween.EaseType.easeOutElastic));

        Invoke("EnableAnimator", 2);
        Invoke("CallIntro2", Sound_Intro1[VOLanguage].clip.length + 2.1f);
        Invoke("CallIntro3", Sound_Intro1[VOLanguage].clip.length+ Sound_Intro2[VOLanguage].clip.length+2.1f);
    }
    ...
    public void BtnAct_OkTut()
    {
        ...
        float LengthDelay = PlayAnswerVoiceOver(1, Sound_BtnOkClick.clip.length);
        ...
        iTween.MoveTo(Tut_Sidepanel.gameObject, iTween.Hash("x",1000, "islocal",true,"time", 1f, "delay", 0, "easetype", iTween.EaseType.easeOutElastic));
    ...
    public void GenerateLevel()
    {
        ...
        Invoke("EnableOptionsRaycast", LengthDelay + 0.5f);
        ...
        iTween.MoveTo(SidePanel.gameObject, iTween.Hash("x", 512, "time", 1f,"islocal",true ,"delay", LengthDelay, "easetype", iTween.EaseType.easeOutElastic));
    ...
    float PlayQuestionVoiceOver(int _Qi)
    {
        ...
        switch (_Qi)
        {
            case 0:
                ...
                ClipLength = Sound_Q1[VOLanguage].clip.length;
                break;
            case 1:
                PlayAudioRepeated(Sound_Q2);
                ClipLength = Sound_Q2[VOLanguage].clip.length;
                break;
        }
        return ClipLength;
    }
    ...
    public void BtnAct_Ok()
    {
        ...
        if (CorrectAnsrIndex == UserAnsr)
        {
            ...
            iTween.MoveTo(SidePanel.gameObject, iTween.Hash("x", 1000,"islocal",true, "time", 1f, "delay", LengthDelay + 1.5f, "easetype", iTween.EaseType.easeOutElastic));
        ...
        else
        {
            ...
            if (WrongAnsrsCount >= 2)
            {
                ...
                iTween.MoveTo(SidePanel.gameObject, iTween.Hash("x", 1000, "islocal", true, "time", 0.25f, "delay", LengthDelay+ Sound_IncorrectAnswer.clip.length+1, "easetype", iTween.EaseType.easeOutElastic));
