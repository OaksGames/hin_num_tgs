...
public class GameManager_A02031 : MonoBehaviour, IOAKSGame
{
    ...
    /* bool Init;
     ...
     private void OnDisable()
     {
         if (!Init)
         {
             MultiLevelManager.instance.LoadProgressMaxValues(QuestionOrder.Length);
             Init = true;
         }
     }
    ...
    public void SetTutorial(GameInputData gameInputData)
    {
        ...
        LevelHolder.transform.parent.gameObject.SetActive(false);
    ...
    {
        TutorialObj.gameObject.SetActive(false);
        LevelHolder.transform.parent.gameObject.SetActive(true);
    ...
    public void BtnAct_Ok()
    {
        ...
        if (CorrectAnsrIndex == UserAnsr)
        {
            ...
            StartCoroutine(SetActiveWithDelayCall(LevelHolder, false, LengthDelay + LengthDelay2 + 2f));
