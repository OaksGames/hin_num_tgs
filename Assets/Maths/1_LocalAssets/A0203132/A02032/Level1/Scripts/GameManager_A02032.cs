...
public class GameManager_A02032 : MonoBehaviour, IOAKSGame
{
    ...
    /* bool Init;
     ...
     private void OnDisable()
     {
         if (!Init)
         {
             MultiLevelManager.instance.LoadProgressMaxValues(QuestionsOrder1.Length + QuestionsOrder2.Length);
             Init = true;
         }
     }
    ...
    {
        float _delay = 0.1f;
        for (int i = 0; i < TutTrayObjs[1].transform.GetChild(0).transform.childCount; i++)
        {
            TutTrayObjs[1].transform.GetChild(0).transform.GetChild(i).GetComponent<PopTweenCustom>().Invoke("StartAnim", _delay);
            _delay += 1;
            TutTrayObjs[1].transform.GetChild(0).transform.GetChild(i).GetComponent<PopTweenCustom>().Invoke("StopAnim", _delay);
        }
    }
    ...
    public void GenerateLevel()
    {
        ...
        if (TraysItems.Length >= 0)
        {
            for (int i = 0; i < Trays.Length; i++)
            {
                Destroy(TraysItems[i].gameObject);
    ...
    public void BtnAct_Ok()
    {
        ...
        if (CorrectAnsrIndex == UserAnsr)
        {
           ...
           // MultiLevelManager.instance.UpdateProgress(1, 1);
            ...
            if (CurrentItem == 0)
            { Invoke("CupCakeAnim", LengthDelay + 1); }
            else
            ...
            if (TraysItems.Length >= 0)
            {
                for (int i = 0; i < Trays.Length; i++)
                {
                    Destroy(TraysItems[i].gameObject, LengthDelay + LengthDelay2 + 2f);
                }
            }
        ...
        else
        {
            ...
            if (WrongAnsrsCount >= 2)
            {
                ...
                if (CurrentItem == 0)
                { Invoke("CupCakeAnim", Sound_IncorrectAnswer.clip.length + 0.75f); }
                else
